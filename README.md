# AGiXT Even Realities

*Note: This project is a fork of the original Even Realities Demo App, modified to integrate AGiXT.*

## AGiXT Chat Completions

The AGiXT Even Realities app integrates AGiXT Chat Completions, enhancing the AI capabilities of the application. This feature allows for more dynamic and context-aware interactions with the AI model.

### Key Features of AGiXT Chat Completions:

- Seamless integration with the existing Even AI functionality
- Enhanced conversation management with context retention
- Support for multiple AI providers through AGiXT's versatile framework
- Improved response generation using AGiXT's advanced prompt templates and chain management

### How it works:

1. When the Even AI function is activated, the app now utilizes AGiXT Chat Completions for processing user queries.
2. The audio input is converted to text and sent to the AGiXT Chat Completions endpoint via the AGiXT Dart SDK.
3. AGiXT processes the input, maintaining conversation context and leveraging its advanced features.
4. The generated response is then sent back to the glasses for display, following the existing protocol for text transmission.

### Configuration:

The AGiXT server URL and optional API Key can be configured within the app via the "Server Config" page (accessible from the settings icon on the home page). The app uses these settings to connect to your AGiXT instance. By default, it attempts to connect to `http://localhost:7437`. For developers looking to extend or modify the AGiXT integration, refer to the `api_services.dart` file in the `lib/services/` directory.


## Even AI (Original Functionality)
...
app needs to convert the audio stream into text, which is then sent to the large model (now AGiXT) for a
